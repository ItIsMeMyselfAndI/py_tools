...
[Algorithm]
	(1) Server modes:
		(a) Shell:
			-> Send shell signal to client.
			Loop:
					-> Receive shell command through input.
					-> Convert command to output.
					-> Send output to client.
		(b) Execute:
			-> Send execute signal to client.
			-> Get command through argument field.
			-> Convert command to output.
			-> Send output to client.
		(c) Upload:
			-> Get output filename through argument field.
			-> Write file to server.
			-> Send upload signal to client.
			-> Send success message to client.
	(2) Client mode:
		(a) Shell:
			-> Receive shell signal from server.
			-> Send command through input.
			-> Receive output from server.
			-> Display output to client.
		(b) Execute:
			-> Receive execute signal from server.
			-> Receive output from server.
		(c) Upload:
			-> Receive upload signal from server.
